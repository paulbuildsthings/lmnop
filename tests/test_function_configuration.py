...
import lmnop.builder.tasks
from lmnop import __version__
from lmnop.builder.tasks import process_function_configuration as parser
    ...
    def test_parser_valid(self):
        ...
        with write_function_definition(definition) as td:
            ...
            self.assertEqual({
                ...
                "provider": {
                    "name": "openfaas",
                    "gateway": lmnop.builder.tasks.LMNOP_GATEWAY,
                },
                "functions": {
                    "test": {
                        "lang": "python3",
                        "handler": os.path.join(td, "source"),
                        "image": f"{lmnop.builder.tasks.LMNOP_REGISTRY}/test:latest",
                        "requests": {
                            "cpu": "50m",
                            "memory": "64Mi",
                        },
